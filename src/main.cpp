...
#if defined(NDEBUG)
# error "Browniecoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Browniecoin Signed Message:\n";
...
{
    // Browniecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Browniecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Browniecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Browniecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Browniecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Browniecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x624126f9dad538ee54fb9f07e64aac6f4cb75252d62efef612e17b9a0c165e36");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ABC NEWS 07/Jan/2021 Facebook indefinitely blocks Trump's account after violence at Capitol";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04aaf22cfc1fc46ec419efc25f2da58356d244765926cd81bf5b90a82d5bcaa5352ec78496883c7bb5df6814b9b7f80e27c215f57cc31bf825efc63e43efa23662") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1610065906;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085758787;

        if (fTestNet)
        {
            block.nTime    = 1610065955;
            block.nNonce   = 2085758787;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x236a2ae420d175490d05943f4e45cf044dd4857099225da8c6c0ba460f59cc33"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xb6, 0xdc }; // Browniecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BrowniecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BrowniecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BrowniecoinMiner(CWallet *pwallet)
{
    printf("BrowniecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("browniecoin-miner");
    ...
    try { loop {
        ...
        printf("Running BrowniecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BrowniecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BrowniecoinMiner, pwallet));
}
